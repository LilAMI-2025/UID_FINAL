...
if st.session_state.page == "home":
    ...
    st.markdown("## 📊 Survey Categorization")
    st.markdown('<div class="data-source-info">📂 <strong>Process:</strong> Analyze survey titles → Categorize questions → Assign UIDs by category</div>', unsafe_allow_html=True)
    
    # Check if we have survey data
    if st.session_state.all_questions is None or st.session_state.all_questions.empty:
        st.markdown('<div class="warning-card">⚠️ No survey data available. Please select surveys first.</div>', unsafe_allow_html=True)
    ...
    with st.spinner("📊 Analyzing survey categories..."):
    ...
    with col1:
        selected_categories = st.multiselect(
            ...
            options=list(SURVEY_CATEGORIES.keys()) + ["Uncategorized"],
            default=list(SURVEY_CATEGORIES.keys()) + ["Uncategorized"]
        )
    
    with col2:
        show_main_only = st.checkbox("Show main questions only", value=True, key="cat_main_only")
    ...
    # Apply filters
    filtered_df = categorized_df[categorized_df['category'].isin(selected_categories)].copy()
    ...
    if not filtered_df.empty:
        ...
        display_columns = [
            "category", "question_uid", "heading_0", "schema_type", 
        ...
        edited_categorized_df = st.data_editor(
            ...
            disabled=["category", "question_uid", "heading_0", "schema_type", "is_choice", "survey_count", "Final_UID"],
            hide_index=True,
            height=500,
        ...
        # Summary by category
        st.markdown("### 📊 Assignment Summary by Category")
        
        assignment_summary = categorized_df.groupby('category').agg({
            'heading_0': 'count',
