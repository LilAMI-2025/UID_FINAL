...
# Enhanced data quality management for handling excessive UID variations
...
def analyze_uid_variations(df_reference):
    """
    Comprehensive analysis of UID variations to identify data quality issues
    """
    analysis_results = {}
    ...
    high_variation_threshold = 50  # UIDs with more than 50 variations
    ...
    for uid in problematic_uids.head(10).index:
        ...
        variation_analysis[uid] = {
            ...
            'sample_questions': unique_questions[:5]  # Show first 5 unique samples
        }
...
def clean_uid_variations(df_reference, cleaning_strategy='aggressive'):
    """
    Clean UID variations based on different strategies
    """
    ...
    if cleaning_strategy == 'conservative':
        ...
        df_cleaned['normalized_question'] = df_cleaned['heading_0'].apply(enhanced_normalize)
    ...
    # Step 3: Handle UIDs with excessive variations
    if cleaning_strategy in ['moderate', 'aggressive']:
        uid_counts = df_cleaned['uid'].value_counts()
        excessive_threshold = 20 if cleaning_strategy == 'moderate' else 10
        ...
        for uid in excessive_uids:
            ...
            if cleaning_strategy == 'moderate':
                # Keep top 20 best questions
                best_questions = sorted(uid_questions, key=lambda q: score_question_quality(q), reverse=True)[:20]
...
def score_question_quality(question):
    ...
    if question[0].isupper() if question else False:
        score += 10
    
    # Avoid artifacts
    bad_patterns = ['click here', 'please select', '...', 'n/a', 'other', 'select one', 'choose all']
...
def create_data_quality_dashboard(df_reference):
    ...
    with col4:
        problematic_pct = (analysis['problematic_uids']['count'] / analysis['unique_uids']) * 100
        st.metric("⚠️ Problematic UIDs", f"{problematic_pct:.1f}%")
    
    # Problematic UIDs section
    if analysis['problematic_uids']['count'] > 0:
        st.markdown("### ⚠️ UIDs with Excessive Variations (>50)")
        
        problematic_df = pd.DataFrame([
            {'UID': uid, 'Variations': count, 'Percentage': f"{(count/analysis['total_questions'])*100:.1f}%"}
        ...
        for uid, details in list(analysis['variation_patterns'].items())[:3]:
            with st.expander(f"🆔 UID {uid} - {details['total_variations']} variations"):
    ...
    if total_junk > 0:
        st.warning(f"⚠️ Found approximately {total_junk:,} questions that could be cleaned up")
        
        cleaning_options = {
            'Conservative': 'Remove only exact duplicates and obvious junk',
            'Moderate': 'Remove duplicates, normalize similar questions, limit variations per UID to 20',
            'Aggressive': 'Keep only the single best question per UID'
        }
...
# Enhanced page section with data quality management
def enhanced_view_question_bank_page():
    """
    Enhanced view question bank page with data quality management
    """
    st.markdown("## 📖 View Question Bank")
    st.markdown("*Complete question repository with quality management tools*")
    ...
    try:
        ...
        if df_reference.empty:
            ...
            max_variations = uid_counts.max()
            
            if max_variations > 100:
                st.warning(f"⚠️ Data quality issue detected: UID {uid_counts.idxmax()} has {max_variations:,} variations!")
                ...
                with col2:
                    if st.button("📊 Continue with Raw Data", use_container_width=True):
                        st.session_state.show_quality_dashboard = False
            
            # Show quality dashboard if requested
            if st.session_state.get('show_quality_dashboard', False):
                df_reference = create_data_quality_dashboard(df_reference)
                st.markdown("---")
            
            # Continue with regular view...
            # [Rest of the view question bank code]
...
# Snowflake Queries
# Fixed Snowflake query functions with proper pagination

def run_snowflake_reference_query_all():
    """
    Fetch ALL reference questions from Snowflake with pagination
    This ensures we get all questions for the View Question Bank
    """
    all_data = []
    limit = 10000
    offset = 0
...
def run_snowflake_reference_query(limit=10000, offset=0):
    """
    Original function for backward compatibility and specific use cases
    """
    query = """
        SELECT HEADING_0, UID
        FROM AMI_DBT.DBT_SURVEY_MONKEY.SURVEY_DETAILS_RESPONSES_COMBINED_LIVE
...
def get_all_reference_questions():
    ...
    This will be used by both View Question Bank and Unique Questions Bank
    """
    return run_snowflake_reference_query_all()

def create_unique_questions_bank(df_reference):
    """
    Enhanced function to create unique questions bank with better selection logic
    """
    if df_reference.empty:
        return pd.DataFrame()
...
# Updated page sections to fix the data fetching issues

# Replace the "unique_question_bank" page section with this:
    ...
    try:
        with st.spinner("🔄 Loading ALL question bank data and creating unique questions..."):
            # Use the new function to get ALL reference data
            df_reference = get_all_reference_questions()
        ...
        if unique_questions_df.empty:
            ...
            if not filtered_df.empty:
                ...
                if not show_variants:
                    display_df = display_df.drop('all_variants', axis=1, errors='ignore')
                else:
...
# Replace the "view_question_bank" page section with this:
elif st.session_state.page == "view_question_bank":
    st.markdown("## 📖 View Question Bank")
    st.markdown("*Complete question repository with ALL variations and UIDs in ascending order*")
    ...
    try:
        ...
        if df_reference.empty:
            ...
            with col4:
                ...
                completeness = (len(df_reference) / (unique_uids * avg_variants)) * 100 if unique_uids > 0 else 0
            ...
            with col3:
                sort_by = st.selectbox("🔄 Sort by", ["UID (ascending)", "UID (descending)", "Question length"])
            ...
            st.markdown(f"### 📋 Question Bank ({len(filtered_df)} questions showing all variations)")
            
            # Add grouping information
            if not filtered_df.empty:
                uid_counts = filtered_df['uid'].value_counts().sort_index()
                st.info(f"💡 Showing variations across {len(uid_counts)} UIDs. UIDs with most variations: {', '.join([f'{uid}({count})' for uid, count in uid_counts.head(5).items()])}")
            ...
            st.download_button(
                ...
                f"complete_question_bank_{uuid4()}.csv",
                "text/csv",
                use_container_width=True
            )
...
# Enhanced Create New Survey Page
    ...
    try:
        ...
        st.markdown(f'<div class="warning-card">❌ Error: {e}</div>', unsafe_allow_html=True)
# Add this as a new page section (around line 1200, before the navigation footer)
    ...
    try:
        ...
        if df_reference.empty:
            ...
            # Use the create_data_quality_dashboard function
