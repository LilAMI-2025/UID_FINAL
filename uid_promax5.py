...
# Updated Reference Heading Texts - UPDATED WITH NEW ITEMS
HEADING_REFERENCES = [
...
def get_best_question_for_uid(variants, occurrence_counts=None):
    """Select the best quality question from a list of variants - UPDATED TO PRIORITIZE HIGHEST AUTHORITY COUNT"""
    try:
        if not variants:
            return None
        ...
        if occurrence_counts and isinstance(occurrence_counts, dict):
            # Sort variants by occurrence count (descending), then by quality score
            variant_scores = []
            ...
            # Sort by: 1) Occurrence count (desc), 2) Quality score (desc), 3) Length (asc)
            variant_scores.sort(key=lambda x: (-x[1], -x[2], len(x[0])))
            return variant_scores[0][0]
        
        # Fallback to quality-based selection if no occurrence counts
...
# Enhanced Question Bank Builder with 1:1 Optimization - UPDATED TO USE HIGHEST AUTHORITY COUNT
def build_optimized_1to1_question_bank(df_reference):
    """Build optimized 1:1 question bank with conflict resolution and deduplication - UPDATED TO PRIORITIZE HIGHEST AUTHORITY COUNT"""
    ...
    try:
        ...
        for uid, group in grouped_by_uid:
            ...
            # Create occurrence count mapping for this UID
            if 'OCCURRENCE_COUNT' in group.columns:
                occurrence_counts = dict(zip(group['HEADING_0'], group['OCCURRENCE_COUNT']))
            else:
                occurrence_counts = {variant: 1 for variant in all_variants}
            
            # Choose the best question with highest authority count
            best_question = get_best_question_for_uid(all_variants, occurrence_counts)
            
            if not best_question:
                continue
            
            # Count occurrences for this UID
            total_occurrences = group['OCCURRENCE_COUNT'].sum() if 'OCCURRENCE_COUNT' in group.columns else len(group)
            best_question_count = occurrence_counts.get(best_question, 1)
        ...
        for norm_question, group in grouped:
            ...
            # Count UIDs for this normalized question - prioritize by best_question_count (authority)
            uid_counts = group.groupby('UID')['best_question_count'].sum().sort_values(ascending=False)
            
            if len(uid_counts) == 0:
                continue
            
            # Get the best question from the highest authority UID
        ...
        if not optimized_df.empty:
            # Sort by winner authority count (descending), then quality score
            optimized_df = optimized_df.sort_values(['winner_count', 'quality_score'], ascending=[False, False])
...
if st.session_state.page == "home":
    ...
    # Filters - Survey category, question type, and QUESTION TYPE FILTER (NEW)
    col1, col2, col3 = st.columns(3)
    with col1:
    ...
    with col3:
        # NEW QUESTION TYPE FILTER - Main Question, Choice, or Heading
    ...
    if question_type_filter != "All":
        if question_type_filter == "Main Question":
            ...
            # Filter for headings based on question classification
            if 'question_category' in filtered_df.columns:
                filtered_df = filtered_df[filtered_df['question_category'] == "Heading"]
            else:
                # Fallback: classify questions as headings if they match heading references
    ...
    # Search functionality - UPDATED WITH SEARCH FILTER
    ...
    if search_query:
        ...
        st.info(f"🔍 Found {len(display_df)} results for '{search_query}'")
