    ...
    try:
        with st.spinner("­Ъћё Loading ALL question bank data and creating unique questions..."):
            ...
            if df_reference.empty:
                ...
                if not unique_questions_df.empty:
                    st.markdown("### РГљ Unique Questions")
                    st.dataframe(unique_questions_df[['uid', 'best_question', 'survey_category', 'total_variants', 'quality_score']], use_container_width=True)
                    
                    # Offer download
                    st.download_button(
                        "­ЪЊЦ Download Unique Questions Bank",
                        unique_questions_df.to_csv(index=False),
                        f"unique_question_bank_{uuid4()}.csv",
                        "text/csv",
                        use_container_width=True
                    )

    with st.expander("­ЪћЇ UID Audit: Dominant UID and Conflicts", expanded=False):
        if 'heading_0' in df_reference.columns and 'uid' in df_reference.columns:
            ...
            st.markdown("### РюЁ Most Common UID Assignment per Question")
            st.dataframe(final_assignments[['heading_0', 'uid', 'count', 'conflict']])

            st.markdown("### Рџа№ИЈ Top Conflicts (UIDs with similar count)")
            ...
            st.download_button("РгЄ№ИЈ Download UID Audit Results", csv_export, "uid_audit_summary.csv", "text/csv")
        else:
            st.warning("Missing 'uid' or 'heading_0' in reference data.")

    except Exception as e:
        st.error(f"РЮї Failed to load question bank: {e}")

# SurveyMonkey Categorized Viewer
with st.expander("­ЪЊі SurveyMonkey: Categorize Questions by Survey Title", expanded=False):
    try:
        ...
        if selected_cat:
            ...
            st.markdown(f"### ­ЪЊї Unique Questions for `{selected_cat}`")
            st.dataframe(unique_qs)

            if st.button("­Ъџђ Trigger UID Assignment for This Category"):
