    ...
    try:
        ...
        if not surveys:
            ...
            if all_selected_survey_ids:
                ...
                with tab1:
                    st.markdown("### 📊 Survey Analysis & Mandatory Settings")
                    ...
                    if st.session_state.df_target.empty:
                        ...
                        st.markdown("---")
                        st.markdown("### ⚙️ Configure Mandatory Status")
                        st.markdown("*Edit which questions are mandatory. Changes will be reflected in the final configuration.*")
                        
                        show_main_only = st.checkbox("📋 Show only main questions", value=True, key="tab1_main")
                ...
                with tab2:
                    if st.session_state.df_final is not None:
                        st.markdown("### 🔗 UID Matching Results & Configuration")
                        ...
                        if matched_percentage == 0.0:
                            st.markdown('<div class="warning-card">⚠️ No matches found. This may be due to excluded questions or Snowflake connectivity issues.</div>', unsafe_allow_html=True)
                        ...
                        col1, col2, col3 = st.columns(3)
                        with col1:
                            show_main_only = st.checkbox("📋 Show only main questions", value=True, key="tab2_main_only")
                        with col2:
                        ...
                        # Enhanced search
                        st.markdown("#### 🔍 Advanced Search & Selection")
                        search_query = st.text_input("🔍 Search questions/choices", placeholder="Type to filter questions...")
                        ...
                        st.markdown(f"#### 📊 UID Configuration ({len(result_df)} items)")
                        
                        if st.session_state.df_final["Final_UID"].isna().all():
                            st.markdown('<div class="info-card">ℹ️ UID matching disabled due to Snowflake issues. Assign UIDs manually or fix the connection.</div>', unsafe_allow_html=True)
                        
                        # UID options for dropdown
                        uid_options = [None]
                        if st.session_state.df_reference is not None:
                            ...
                            st.markdown('<div class="warning-card">⚠️ UID options unavailable due to Snowflake issues. Fix connection to load UIDs.</div>', unsafe_allow_html=True)
                        
                        # Display and edit UID assignments
                        result_columns = ["survey_id_title", "heading_0", "position", "is_choice", "Final_UID", "Final_Match_Type", "question_uid", "schema_type", "question_category", "Change_UID"]
                        result_columns = [col for col in result_columns if col in result_df.columns]
                        display_df = result_df[result_columns].copy()
                        display_df = display_df.rename(columns={"heading_0": "Question/Choice", "Final_UID": "current_UID"})
                        ...
                        edited_df = st.data_editor(
                            ...
                            column_config={
                                ...
                                "position": st.column_config.NumberColumn("Position", width="small"),
                                "is_choice": st.column_config.CheckboxColumn("Is Choice", width="small"),
                                "current_UID": st.column_config.TextColumn("Current UID", width="small"),
                                "Final_Match_Type": st.column_config.TextColumn("Match Type", width="small"),
                                "question_uid": st.column_config.TextColumn("Question UID", width="small"),
                                "schema_type": st.column_config.TextColumn("Schema Type", width="small"),
                                "question_category": st.column_config.TextColumn("Category", width="small"),
                                "Change_UID": st.column_config.SelectboxColumn(
                                    "🔄 Assign New UID",
                                    help="Select a UID from Snowflake database",
                                    options=uid_options,
                                    default=None,
                                    width="medium"
                                )
                            },
                            disabled=["survey_id_title", "Question/Choice", "position", "is_choice", "current_UID", "Final_Match_Type", "question_uid", "schema_type", "question_category"],
                            hide_index=True,
                            use_container_width=True
                        )
                        ...
                        for idx, row in edited_df.iterrows():
                            if idx < len(st.session_state.df_final):
                        ...
                        with col1:
                            ...
                            st.markdown("Submit new questions via Google Form:")
                            st.markdown("📝 [Submit New Question](https://docs.google.com/forms/d/1LoY_La59UJ4ZsuxckM8Wl52kVeLI7a1t1MF8zIQxGUs)")
                            st.markdown("🆔 [Submit New UID](https://docs.google.com/forms/d/1lkhfm1-t5-zwLxfbVEUiHewveLpGXv5yEVRlQx5XjxA)")
                        
                        with col2:
                            st.markdown("**✏️ Customize Questions**")
                            st.markdown("Create customized versions of existing questions:")
                            ...
                            with st.expander("🔧 Add Custom Question"):
                                ...
                                customize_edited_df = st.data_editor(
                                    ...
                                    column_config={
                                        "Pre-existing Question": st.column_config.SelectboxColumn(
                                            ...
                                            help="Choose from current survey questions",
                                            options=question_options,
                                            default=None
                                        ),
                                        "Customized Question": st.column_config.TextColumn(
                                            "Enter Custom Text",
                                            help="Type your customized question",
                                            default=""
                                        )
                                    },
                                    hide_index=True,
                                ...
                                for _, row in customize_edited_df.iterrows():
                                    if row["Pre-existing Question"] and row["Customized Question"]:
                                        ...
                                        if custom_question and custom_question not in st.session_state.custom_questions["Customized Question"].values:
                        ...
                        if not st.session_state.custom_questions.empty:
                            st.markdown("#### ✏️ Your Custom Questions")
                            st.dataframe(
                                st.session_state.custom_questions,
                                column_config={
                                    "Customized Question": st.column_config.TextColumn("Custom Question", width="large"),
                ...
                with tab3:
                    if st.session_state.df_final is not None:
                        st.markdown("### ✅ Final Configuration & Export")
                        ...
                        col1, col2, col3, col4 = st.columns(4)
                        with col1:
                            st.metric("🎯 Final Match Rate", f"{matched_percentage}%")
                        with col2:
                            mandatory_count = len(st.session_state.df_final[st.session_state.df_final["mandatory"] == True])
                            st.metric("⚡ Mandatory Questions", mandatory_count)
                        with col3:
                            configured_uids = len(st.session_state.df_final[st.session_state.df_final["configured_final_UID"].notna()])
                            st.metric("🔗 Configured UIDs", configured_uids)
                        with col4:
                            custom_count = len(st.session_state.custom_questions)
                            st.metric("✏️ Custom Questions", custom_count)
                        
                        if matched_percentage < 50:
                            st.markdown('<div class="warning-card">⚠️ Low match rate detected. Consider reviewing unmatched questions or submitting new UIDs.</div>', unsafe_allow_html=True)
                        elif matched_percentage >= 80:
                            st.markdown('<div class="success-card">🎉 Excellent match rate! Your survey is well-configured.</div>', unsafe_allow_html=True)
                        
                        st.markdown("---")
                        
                        # Final configuration preview
                        st.markdown("#### 📋 Final Survey Configuration")
                        
                        config_filter = st.selectbox("🎯 View Configuration", 
                                                   ["All Items", "Main Questions Only", "Mandatory Only", "With UIDs Only"])
                        ...
                        # Enhanced preview for Snowflake upload
                        ...
                        st.markdown('<div class="info-card">ℹ️ Select and process a survey to view the final configuration.</div>', unsafe_allow_html=True)
            else:
